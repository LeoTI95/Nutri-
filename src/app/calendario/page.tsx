...
// 🎯 NOVA FUNÇÃO HELPER
// Formata um objeto Date local para YYYY-MM-DD sem converter fuso
...
export default function CalendarioPage() {
  ...
  useEffect(() => {
    ...
    else setFilteredAppointments([]); // Limpa os agendamentos se nenhuma data for selecionada
  ...
  const getDateRange = () => {
    ...
    // 🎯 CORREÇÃO: Usar formatador local
    return {
  ...
  /* ============================================================
     Navegação de data
  ============================================================ */
  // 🎯 CORREÇÃO: Adicionada função que faltava
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refiltra os agendamentos para o dia selecionado
      if (selectedDate) {
        const dateStr = formatLocalDate(selectedDate);
        // Recarrega os agendamentos e *depois* filtra
        // O useEffect [selectedDate, appointments] já fará isso
      }
  ...
  const openModal = (date?: Date) => {
    // 🎯 CORREÇÃO: Usar formatador local para data clicada, ou 'hoje' (TZ)
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className={`grid gap-2 grid-cols-7`}>
        {days.map((day, idx) => {
          // 🎯 CORREÇÃO: Usar formatador local
          const dateStr = formatLocalDate(day); // YYYY-MM-DD
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          <button
                            onClick={() => { 
                              // Ao editar, garante que o form data está correto
                              const aptData = {
                                ...apt,
                                date: apt.date, // Garante que a data YYYY-MM-DD do banco seja usada
                                time: apt.time
                              };
                          ...
                          <button
                            onClick={async () => {
                              ...
                              await loadAppointments();
                              // O useEffect [selectedDate, appointments] cuidará da re-filtragem
                            }}
