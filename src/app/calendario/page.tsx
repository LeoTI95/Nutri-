...
// 1. Função que formata o instante de tempo para YYYY-MM-DD no TIMEZONE
function toDateStrTZ(d: Date) {
  return new Intl.DateTimeFormat('en-CA', { timeZone: TIMEZONE }).format(d);
}

// 2. Retorna a string de 'hoje' no TIMEZONE (usado para o botão 'Hoje' e data inicial)
function todayStrTZ() {
  return toDateStrTZ(new Date());
}

// 3. 🎯 CRÍTICO: Função que formata um objeto Date (data local) para YYYY-MM-DD SEM conversão de fuso
// Esta deve ser usada para datas clicadas no calendário (evita o bug de -1 dia)
function formatLocalDate(d: Date) {
  return new Intl.DateTimeFormat('en-CA').format(d);
}

// 4. Função segura para criar Date local a partir de YYYY-MM-DD
...
export default function CalendarioPage() {
  ...
  const [serverTime, setServerTime] = useState(''); // Estado para o horário do servidor (BRT)
  ...
  /* ============================================================
     Hook para atualizar o horário do servidor (BRT)
  ============================================================ */
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      // Formata a data e hora para o fuso America/Sao_Paulo (BRT)
    ...
    updateTime(); // Roda na inicialização
    const timerId = setInterval(updateTime, 1000); // Roda a cada segundo

    return () => clearInterval(timerId); // Limpeza
  }, []);
  ...
  const getDateRange = () => {
    ...
    // 🎯 CORREÇÃO 1: Usar formatador local (sem TZ) para as strings de busca no Supabase.
    return {
  ...
  const filterAppointmentsByDate = (date: Date) => {
    // 🎯 CORREÇÃO 2: Usar formatador local
  ...
  const openModal = (date?: Date) => {
    // 🎯 CORREÇÃO 3: Usar formatador local (sem fuso) para data clicada.
    const d = date ? formatLocalDate(date) : todayStrTZ();
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className={`grid gap-2 grid-cols-7`}>
        {days.map((day, idx) => {
          // 🎯 CORREÇÃO 4: Usar formatador local para o dia do loop.
          const dateStr = formatLocalDate(day);
          const items = appointments.filter(a => a.date === dateStr);
          const isToday = dateStr === todayStr;
          
          // 🎯 CORREÇÃO 4 (cont.): Usar formatador local para o dia selecionado.
          const isSelected = selectedDate && dateStr === formatLocalDate(selectedDate);
  ...
  const [serverDatePart, serverTimePart] = serverTime.split(',').map(s => s.trim());
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                {/* Título Principal (Alterado para Meus Atendimentos) */}
                <div>
                ...
                {/* Horário do Servidor (BRT) */}
                {serverTime && (
                    <div className="text-left bg-gray-100 dark:bg-gray-800 p-2 rounded-lg text-sm border border-gray-200 dark:border-gray-700">
                        <p className="font-semibold text-gray-700 dark:text-gray-300">Horário de Brasília (BRT)</p>
